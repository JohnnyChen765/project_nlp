...
class SkipGram:
    def __init__(
        ...
        negative_sample_proba = self.create_negative_sample_probabilities(list(self.word2occurences.values()))
        self.word2negative_sampling_probabilities = dict(zip(self.word2occurences.keys(), negative_sample_proba))
...
​
​
    def sample(self, omit, n_sampling = 5):
        ...
        probabilities = self.create_negative_sample_probabilities(list(words_not_omitted.values()))
        words = list(words_not_omitted.keys())
        # words = list(self.word2negative_sampling_probabilities.keys())
        # probabilities = list(self.word2negative_sampling_probabilities.values())
        ...
        while len(random_values) != 0:
            ...
            negative_words.append(words[cursor_proba])
...
​
	...
	#A ameliorer plus tard si besoin.
        ...
        for w in self.word2negative_sampling_probabilities:
            file_sp.write(w+','+str(self.word2negative_sampling_probabilities[w])+"\n")
        ...
        file_param.write(str(self.total_number_of_words)+"\n"+str(self.negative_rate)+"\n"+str(self.window_size))
​
